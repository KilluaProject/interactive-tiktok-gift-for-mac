...
connection.on("gift", (data) => {
  ...
  if (data.giftName.toLowerCase() === "doughut") {
    console.log("Menerima gift doughut, menekan tombol P, U, dan K.");

    // Eksekusi tombol satu per satu dengan penundaan
    setTimeout(() => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        // Selesai memproses gift, reset status
        isProcessingGift = false;
      }, 200);
    }, 1000);
  } else {
    // Jika bukan gift coffee, reset status eksekusi
    isProcessingGift = false;
  }

  if (data.giftName.toLowerCase() === "heart me") {
