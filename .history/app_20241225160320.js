...
connection.on("gift", (data) => {
  ...
  // Gift untuk nengok belakang (coffee)
  ...
  // Gift Hi Bear untuk memutar Indonesia Raya
  if (data.giftName.toLowerCase() === "mishka bear") {
    if (!hasPlayedIndonesiaRaya) {
      console.log("Menerima gift mishka bear, memutar lagu Indonesia Raya.");

      // Memutar lagu Indonesia Raya
      player.play("./Sound/indonesiaraya.mp3", (err) => {
  ...
  if (data.giftName.toLowerCase() === "confetti") {
    if (!hasPlayedIndonesiaRaya) {
      ...
      setTimeout(() => {
        robot.keyToggle("n", "up"); // Lepaskan tombol 'n' setelah 0.5 detik
      }, 500);
