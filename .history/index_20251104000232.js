...
const giftMap = require("./gifts.json");
let connection = null;
...
async function connectTikTok() {
  const { username } = await inquirer.prompt({
    type: "input",
    name: "username",
    message: "Masukkan username TikTok (atau ketik 'kembali' untuk menu utama):",
  });

  if (username.toLowerCase() === "kembali") return; // kembali ke menu utama

  const { confirm } = await inquirer.prompt({
    type: "list",
    name: "confirm",
    message: `Anda yakin sudah benar memasukkan akun TikTok Anda: @${username}?`,
    choices: ["Yes", "No"],
  });

  if (confirm === "No") return connectTikTok(); // input ulang username
...
async function manageGift() {
  ...
  while (!exit) {
    ...
    if (action === "Tambah/Update Gift") {
      ...
      saveGifts();
      console.log(`✅ Gift '${name}' berhasil disimpan!`);
      ...
      delete giftMap[name];
      saveGifts();
      console.log(`🗑 Gift '${name}' berhasil dihapus!`);
