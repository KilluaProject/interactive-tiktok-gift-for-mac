...
connection.on("gift", (data) => {
  ...
  if (data.giftName.toLowerCase() === "coffee") {
    console.log(
      "Menerima gift finger heart, memutar suara dan menekan tombol S selama 5 detik."
    );
  ...
  if (data.giftName.toLowerCase() === "gg") {
    ...
    setTimeout(() => {
      ...
      console.log("Tombol m dilepas (up)");
    }, 3000);
  }
  // Gift Rem
  if (data.giftName.toLowerCase() === "coffee") {
  ...
  if (data.giftName.toLowerCase() === "doughnut") {
    ...
    console.log("Tombol t ditekan (down)");

    setTimeout(() => {
      robot.keyToggle("t", "up");
      console.log("Tombol t dilepas (up)");
    }, 500);
  }
  ...
  if (data.giftName.toLowerCase() === "corgi" && !isProcessing) {
    ...
    setTimeout(() => {
      ...
      isProcessing = false; // Reset setelah selesai
    }, 500);
  }
});
