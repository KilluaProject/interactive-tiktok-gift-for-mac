...
connection.on("gift", (data) => {
  ...
  //rewind
  if (data.giftName.toLowerCase() === "finger heart" && !isProcessing) {
    isProcessing = true;
    console.log("Memproses gift...");

    // Logika keyboard terlebih dahulu
    robot.keyToggle("y", "down");
    console.log("Tombol Y ditekan (down)");

    setTimeout(() => {
      robot.keyToggle("y", "up");
      console.log("Tombol Y dilepas (up)");

      // Setelah tombol dilepas, setel musik
      player.play("./sound/fart.mp3", (err) => {
        if (err) {
        ...
        // Reset hanya setelah musik selesai diputar
        setTimeout(() => {
          isProcessing = false; // Reset state untuk memungkinkan eksekusi berikutnya
          console.log("Siap memproses gift berikutnya.");
        }, 500); // Delay tambahan untuk memastikan stabilitas
      });
    }, 100);
  }
  ...
  //restart
  if (data.giftName.toLowerCase() === "perfume" && !isProcessing) {
    isProcessing = true;
    console.log("Memproses gift...");
    ...
    setTimeout(() => {
      ...
      // Setelah tombol dilepas, setel musik
      player.play("./sound/restart.mp3", (err) => {
        if (err) {
          console.error("Error memutar suara:", err);
        } else {
          console.log("Suara diputar dengan sukses.");
  ...
  //Reset Emu
  if (data.giftName.toLowerCase() === "confetti" && !isProcessing) {
    isProcessing = true;
    console.log("Memproses gift...");
